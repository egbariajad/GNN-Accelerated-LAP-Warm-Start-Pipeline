"""Training loop for the lightweight OneGNN model using row-only features."""
...
# Suppress harmless PyTorch scheduler warning
warnings.filterwarnings("ignore", ".*lr_scheduler.step.*optimizer.step.*", UserWarning)
...
from gnn import OneGNN, compute_row_features, ROW_FEATURE_DIM
...
def create_collate_fn():
    """Create a collate function that materialises 21D row features."""

    feature_fn = compute_row_features
    feature_dim = ROW_FEATURE_DIM
    ...
    def collate(batch: Sequence[Dict[str, np.ndarray]]) -> Batch:
        ...
        row_feat = torch.zeros(bsz, max_n, feature_dim, dtype=torch.float32)
        ...
        for i, item in enumerate(batch):
            ...
            features = feature_fn(item["cost"])
            row_feat[i, :n, :] = torch.from_numpy(features)
            mask[i, :n] = True

        return Batch(cost=cost, u=u, v=v, row_feat=row_feat, mask=mask)
...
def collate(batch: Sequence[Dict[str, np.ndarray]]) -> Batch:
    return create_collate_fn()(batch)
...
def compute_loss(batch: Batch, preds: Dict[str, torch.Tensor]) -> tuple[torch.Tensor, Dict[str, list[float]]]:
    ...
    # Use safer big value (was 1e9, which overflows in FP16)
    big = batch.cost.new_tensor(1e6)
    ...
    # Combined loss (standard weights for stability)
    loss = (
        primal_gap.mean()
        + feas.mean()            # Standard feasibility weight  
        + 0.1 * u_reg.mean()     # Standard u-MSE regularization
    )
...
@torch.no_grad()
def evaluate(model: OneGNN, loader: DataLoader, device: torch.device) -> Dict[str, float]:
    model.eval()
    total_loss = 0.0
    count = 0
    primal_values: list[float] = []
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output", type=Path, default=Path("gnn/checkpoints/one_gnn.pt"))
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    # Always use the 21D feature set
    feature_dim = ROW_FEATURE_DIM
    collate_fn = create_collate_fn()

    print(f"Using 21D features ({feature_dim}D)")
    ...
    train_loader = DataLoader(
        LapDataset(args.train), batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn
    )
    val_loader = (
        DataLoader(LapDataset(args.val), batch_size=args.batch_size, collate_fn=collate_fn)
        if args.val
        else None
    )
    ...
    # Disable AMP to prevent FP16 overflow with large feature values
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_stats is not None:
            ...
            if current_gap < best_val_gap - 1e-6:
                ...
                patience += 1
                if epoch >= args.min_epochs and patience >= args.patience:
