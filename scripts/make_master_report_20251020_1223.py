#!/usr/bin/env python3
import argparse, glob, io, math, re
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Auto-build a benchmarking PowerPoint from CSV logs."
    )
...
def normalize_columns(df):
    ...
    # best-effort dtype for descriptors
    for c in ["gpu_name","dtype","pinned","__source_file"]:
        if c in out.columns:
            out[c] = out[c].astype(str)

    # ensure integer n when possible
...
def add_picture_slide(prs, title, img_bytes, width_inches=9.0):
    ...
    slide.shapes.title.text = title
    pic = slide.shapes.add_picture(img_bytes, Inches(0.5), Inches(1.2), width=Inches(width_inches-1.0))
    return slide
...
def section_txfer(prs, df, dpi):
    need = [c for c in ["T_H2D_ms","BW_H2D_GBs","T_D2H_ms","BW_D2H_GBs"] if c in df.columns]
    if "size_n" not in df.columns or not need:
        return
...
def section_solver(prs, df, dpi):
    ...
    if "size_n" not in df.columns or not has_any:
        return
...
def section_meta(prs, df, dpi):
    # Show dataset coverage & metadata if available
    rows = []
...
def main():
    ...
    if df.empty or "size_n" not in df.columns:
        ...
        # Raw counts by file (optional helpful page)
