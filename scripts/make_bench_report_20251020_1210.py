...
def coerce_numeric(df):
    ...
    for c in out.columns:
        ...
        if out[c].dtype.kind in "biufc":
            continue
        # only coerce common numeric performance columns
...
def numeric_groupby(df, by):
    ...
    keep = [c for c in num.columns if c != by]
    grouped = df.groupby(by)[keep].agg(["median","mean","count"])
...
def add_picture_slide(prs, title, png_bytes, width_inches=10.0):
    ...
    slide.shapes.title.text = title
    pic = slide.shapes.add_picture(io.BytesIO(png_bytes), Inches(0.5), Inches(1.3), width=Inches(width_inches))
    return slide
...
def plot_txfer(df, size_col):
    ...
    # Summary table (median only for common columns)
    keep_cols = [size_col]
...
def main():
    ...
    # Runtime & speedup slides
    rt = plot_runtime(df, size_col)
    if rt and rt[0] is not None:
        add_picture_slide(prs, "Runtime components", rt[0])
    if rt and rt[1] is not None:
        add_picture_slide(prs, "Speedup vs SciPy", rt[1])
    if rt and rt[2] is not None:
        add_table_slide(prs, "Runtime summary (median)", rt[2])
