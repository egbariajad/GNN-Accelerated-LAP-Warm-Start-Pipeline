...
# add project root to sys.path so we can import features.py and one_gnn.py from parent of scripts/
...
def find_samples(data_root: Path, size: int, per_size: int):
    ...
    if data_root.is_file() and data_root.suffix == ".h5":
        ...
        for p in sorted(data_root.rglob("*.h5")):
            needed = per_size - len(pairs)
            if needed <= 0:
                break
            pairs += load_h5_indices(p, size, needed)
